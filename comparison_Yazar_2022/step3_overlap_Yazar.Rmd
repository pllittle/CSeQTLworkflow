...
for (i in 1:length(ot_cell_types)){
  ...
  ot_i = ot_ct_result[1:min(500,nrow(ot_ct_result)),]
  ot_eGenes_i  = ot_i$Gene.Ensembl.ID
  ...
  for (j in 1:length(cs_cell_types)){
    ...
    cs_j         = cs_ct_result[cs_ct_result$eQTL=="YES",]
    if(nrow(cs_j) > 300){ cs_j = cs_j[1:300,] }
    
    cs_eGenes_j  = cs_j$ENSG
    ...
    ot_ij = ot_i[ot_i$Gene.Ensembl.ID %in% overlap_considered,]
    cs_ij = cs_j[cs_j$ENSG %in% overlap_considered,]

    names(ot_ij)[which(names(ot_ij) =="Gene.Ensembl.ID")] = "ENSG"
    names(ot_ij)[which(names(ot_ij) =="rho.correlation.coefficient")] = "rho"

    ot_cols = c("ENSG", "SNP", "Chromosome", "Position", "rho", 
                "SNP.assessed.allele", "qvalue")
    cs_cols = c("ENSG", "SNP", "ETA", "qvalue")
    
    intersection_ij = merge(ot_ij[,ot_cols], cs_ij[,cs_cols],
                            by="ENSG", suffixes = c("_ot","_cs"))
    intersection_ij[["cell_type_ot"]] = ot_ct_str
    intersection_ij[["cell_type_cs"]] = cs_ct

    if(ot_ct %in% ct_groups[[cs_ct]]){
      intersection_res = rbind(intersection_res, intersection_ij)
    }
...
dim(intersection_res)
intersection_res[1:2,]
table(paste(intersection_res$cell_type_ot, 
            intersection_res$cell_type_cs, sep="-"))
...
write.csv(intersection_res, row.names = FALSE, 
        file=sprintf("results/step3_%s_eGenes_overlap.csv", config))
